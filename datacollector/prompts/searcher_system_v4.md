...
- Use exactly one query in each `search` tool action. A search action must serve
  one task only. Never bundle queries from different tasks in one action. If
  several tasks need searches, perform separate search actions for them.
